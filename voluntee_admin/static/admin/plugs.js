...
define(['jquery'], function () {
    ...
    msg.prototype.confirm = function (msg, ok, no) {
        var self = this;
        var index = layer.confirm(msg, {btn: ['确认', '取消']}, function () {
    ...
    msg.prototype.success = function (msg, time, callback) {
        this.close();
        this.closeAutoSuccessCloseIndex();
    ...
    msg.prototype.closeAutoSuccessCloseIndex = function () {
        var self = this;
        for (var i in self.autoSuccessCloseIndexs) {
            layer.close(self.autoSuccessCloseIndexs[i]);
        }
     ...
     * @param type
     */
    _form.prototype.modal = function (url, data, title, callback, loading, tips, type) {
        layer.closeAll();

        this.load(url, data, type ? type : 'GET', function (res) {
            if (typeof (res) === 'object') {
                return $.msg.auto(res);
            }
            layer.open({
