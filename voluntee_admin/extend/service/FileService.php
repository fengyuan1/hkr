...
class FileService
{
    ...
    public static function getFileUrl($filename, $storage = null)
    {
        ...
        switch (empty($storage) ? config('qiniu.storage_type') : $storage) {
            case 'local':
    ...
    {
        $region = config('qiniu.storage_qiniu_region');
        $isHttps = !!config('qiniu.storage_qiniu_is_https');
        switch ($region) {
            case '华东':
                if ($isHttps) {
    ...
    {
        return (config('qiniu.storage_qiniu_is_https') ? 'https' : 'http') . '://' . config('qiniu.regpub_domain') . '/';
    }
    ...
    public static function hasFile($filename, $storage = null)
    {
        switch (empty($storage) ? config('qiniu.storage_type') : $storage) {
            ...
            case 'qiniu':
                ...
                list($ret, $err) = $bucketMgr->stat(config('qiniu.pub_bucket'), $filename);
                return $err === null;
            case 'oss':
    ...
    {
        $type = empty($file_storage) ? config('qiniu.storage_type') : $file_storage;
    ...
    {
        $auth = new Auth(config('qiniu.access_key'), config('qiniu.secret_key'));
        $token = $auth->uploadToken(config('qiniu.pub_bucket'));
